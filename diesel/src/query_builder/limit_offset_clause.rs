...
use backend::{Backend, SupportsLonelyOffset};
...
pub struct BoxedLimitOffsetClause<'a, DB> {
    pub(crate) limit: Option<Box<QueryFragment<DB> + 'a>>,
    pub(crate) offset: Option<Box<QueryFragment<DB> + 'a>>,
}
...
where
    L: QueryFragment<DB>,
    O: QueryFragment<DB>,
    DB: Backend + ::backend::SupportsLonelyOffset,
{
    fn walk_ast(&self, mut out: AstPass<DB>) -> QueryResult<()> {
